(() => {
  ...
  const tracks = site.compare.tracks.map((track, index) => `
    <article class="comparison-track" id="track-${esc(track.id)}">
      ...
      <div class="track-analysis"><div><small>Technical shift</small><p>${esc(track.technicalShift)}</p></div><div><small>Remaining bottleneck</small><p>${esc(track.bottleneck)}</p></div><div><small>Official reading</small><div class="mini-paper-links">${track.papers.map(paper => `<a href="${esc(paper.url)}" target="_blank" rel="noreferrer">${esc(paper.label)} ↗</a>`).join('')}</div></div></div>
    </article>`).join('');
  mount.innerHTML = `
